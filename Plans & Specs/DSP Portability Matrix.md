...
| 52 | **BaySickSolsticeProcessor** | Shell for the additive synth, prefix `tk_{id}_bso_`; also owns `registerModTargets`, which *defines* the mod-matrix target ordering. | **REWORK** (shell only).  **Known perf debt:** the shape setter rebuilds a wavetable via FFT from the APVTS sweep, i.e. on the audio thread - already marked TODO in the header. | Ours + JUCE |
| 53 | **BaySickSolsticeSynth** | 16 additive voices sharing two harmonic engines (Part A / B).  Wavetables are read-only at audio time so sharing needs no lock; param changes broadcast and land via SmoothedValues. | **CLEAN** | Ours + JUCE |
...
BaySickSynth, BaySickBass and BaySickSolstice.  **Every row is CLEAN.**
