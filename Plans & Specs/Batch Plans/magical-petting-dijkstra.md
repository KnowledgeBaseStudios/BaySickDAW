...
| Sub-R | **Sub-A scope amendment 2026-05-27**: targeted vendored-sfizz patches are ALLOWED in QA-SfzGroup when the fix is bounded (single-symbol type change, no state-machine logic rewrite).  Sub-A's original spirit (no full sfizz state-machine patching mixed with parser work) preserved — atomic-vs-int is a 1-symbol type change, not state-machine logic.  Specifically: the `libs/sfizz/src/sfizz/Layer.h:151` `int sequenceCounter_ { 0 };` → `std::atomic<int> sequenceCounter_ { 0 };` patch lands in QA-SfzGroup to fix the MT-mode RR race that produced multi-region overlay on cymbals/hi-hats (bit-crusher symptom surfaced 2026-05-27 mid-Task-3 testing). | Jeff verbatim 2026-05-27: "Option (a). Amend Sub-A. A targeted, 1-line type change to fix a thread-safety RR bug absolutely belongs in this QA-SfzGroup batch."  Per `feedback_qa_batches_fix_bugs_dont_defer.md` — real bugs surfaced mid-batch get fixed in-batch by default, especially when directly related to the batch's core mandate (SFZ RR behavior). |
| Sub-S | **`std::memory_order_relaxed`** for the `sequenceCounter_` atomic increment.  The counter is monotonic + isn't synchronizing any other memory state; relaxed is the only appropriate choice for the audio thread.  Implementation: call sites at `Layer.cpp:61` + `:185` use `sequenceCounter_.fetch_add(1, std::memory_order_relaxed)` explicitly — NOT `sequenceCounter_++` (which on `std::atomic` defaults to `std::memory_order_seq_cst` and would defeat the relaxed choice). | Jeff verbatim 2026-05-27: "Option (a) std::memory_order_relaxed. This is a monotonic counter that isn't synchronizing other memory states, so relaxed is the only appropriate choice for the audio thread."  Implementation note Jeff-supplied: "you cannot just leave the call sites as sequenceCounter_++ (which defaults to seq_cst). You must update the two increment sites in Layer.cpp (lines 61 and 185) to use sequenceCounter_.fetch_add(1, std::memory_order_relaxed)." |
