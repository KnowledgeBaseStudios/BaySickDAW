...
| S5 | On-disk sync is Claude's: copy the regenerated `Presets/BaySickSolstice/` + `Templates/Factory/` into `Documents/BaySickDAW/`, delete `Documents/BaySickDAW/Presets/Harmless/`. **Found moot at T1 (2026-09-02): the repo root IS `Documents/BaySickDAW` (`AppPaths::appRoot()` = `userDocumentsDirectory/BaySickDAW`), so the app reads the repo's `Presets/` and `Templates/` directly - `git rm` + the regenerated folder landing in the repo IS the disk sync.  No separate copy exists.** | Jeff item 2a. Satisfied by the repo edit itself. |
