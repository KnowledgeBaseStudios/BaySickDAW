...
- **T7:** floors table in `StandaloneEditor.cpp` (:13033-13036, aux call sites :13158/:13183/:13205), `Source/BaySickSolstice/BaySickSolsticeEditor.*`, `Source/VibePlayer/VibePlayerEditor.cpp` (:6-15, :426-433), `Source/Standalone/EffectEditorPanels.cpp` (pedal-mode branches, :4888-4915 pattern)
- **T8:** per D1/D2 ruling
...
- [ ] Reorder `showAddMenu` to the locked list: BaySickVocal · BaySickLiveInst · BaySickGuitars · BaySickBasses · VSTPlugin (submenu) · BaySickSolstice > Layers/Bass · BaySickSynth (flat — Layers only once Drums leaves) · BaySickPlayer > Layers/Bass/Audio Clips · BaySickBass · **BaySickDrums > BaySickPlayer/BaySickSynth** (new; absorbs the Drums targets from rows 2–3) · BaySickRustyDrums.
...
- [ ] Window-3: `BaySickPresetButton` relocated to the title strip for VibePlayer, BaySickSynth, BaySickBass, BaySickSolstice, Pedals board; RustyDrums' bar-hosted Player Preset + Load Player combo move to its strip; editors reclaim the freed internal-bar height.
...
- [ ] Specific-2 BaySickSolstice full rework (respecting the Part A/B dual-bind + `rebindToPart` machinery — both parts always render; automation stays parameter-addressed).
...
### Task 16 — Sizing model rework + strip consolidation + BaySickSolstice re-layout (Jeff-directed mid-batch 2026-08-04; shipped `3cfdf4c2`)
