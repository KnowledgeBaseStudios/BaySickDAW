...
- **Slip-edit UI affordance (Jeff-locked 2026-05-19 — IN scope for Task 0c, gated by Ctrl+Alt+Home mode toggle):** Builder-grid `ArrangementGrid` (`Source/Standalone/BuilderPage.cpp`) gains a left-edge slip-edit mode — dragging an Audio clip's left edge decrements `block.contentStartSamples` (reveals pre-roll) and grows `block.lengthBeats` accordingly (or increments + shrinks for the trim direction); the clip's right-end and `startBar` stay fixed (FL/Reaper-standard "slip in-point" semantics; only the in-point + visible length move). **Trigger keybind: `Ctrl+Alt+Home` toggles a "resize-from-left-edge" mode** — Task 0c wires this for `ArrangementGrid` (`mResizeFromLeftEdge`-style flag + `nearLeftEdge` + `mSlipEditing` state + mouseDown/Drag/Up branches gated on the mode flag + cursor hint at left edge when on + undo via `beginEdit("Slip")`/`commitEdit()`). D-7 (Piano Roll features bundle, `Files For Claude/Keybinds & Feature Plan.txt:96`) will later wire the SAME keybind for piano-roll note left-edge resize — Task 0c implements the keybind globally + the ArrangementGrid surface; D-7 extends it to the Piano-Roll surface.
