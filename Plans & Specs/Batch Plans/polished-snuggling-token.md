...
- **MIDI placement (Jeff-locked 2026-05-19 — FL "Noodling" + "Early Strike" + input-quantize rules):** compute `preRollBeats = preRollSamples × bpm / (60 × sampleRate)`; shift each captured note's `startBeat` by `−preRollBeats`. **"Noodling" rule (DISCARD):** if a note's `endBeat ≤ 0` after the shift (started AND ended before the downbeat — practicing during count-in), drop it. **"Early Strike" rule (CLAMP):** if a note's `startBeat < 0` but `endBeat > 0` (held early strike, ringing past the downbeat), clamp `startBeat = 0` and recompute `durationBeats = endBeat − 0`. The hard wall is the downbeat — no notes exist before beat 0 in the recorded pattern. **Input quantization (as-shipped in Task 0c):** new global APVTS param `record_quantize_div` (Int 0..5 = Off / 1/4 / 1/8 / 1/16 / 1/32 / 1/64 — 6 values, matching the 6 listed labels; locked spec text above said 0..6 but the literal label list had 6 items so source went with 0..5) exposed via a **new "Global Record-Quantize ▸" submenu added to the existing Record-button dropdown** (the one that currently toggles ASIO / MIDI mode in `GlobalTransportBar`); when non-Off, MIDI commit snaps clamped startBeats to the grid divisor AFTER the Early-Strike clamp. **Superseded by QA-Ee:** the Option (iii) pivot 2026-05-20 reframed this as `Unified_RecordQuantizeDiv` (Int 0..9, 10 triplet-aware labels: Off / Bar / Beat / 1/2 Beat / 1/3 Beat / Step / 1/2 Step / 1/3 Step / 1/4 Step / 1/6 Step) on a 96 PPQ tick foundation; the Task 0c implementation here is the interim binary-snap shape, kept in tree until QA-Ee lands.  See Main Plan §5 QA-Ee + §9 Forks entry.
- **Strip recorders (Jeff-locked 2026-05-19 — IN scope for Task 0c, not deferred):** the same `mPreRollSamples` global value applies to every recorder open during a given Record session. `commitRecordingResult` applies `contentStartSamples` (+ shortened `lengthBeats`) to EVERY Audio block created — master fallback `dropWavAsClip` AND per-strip Vox dry/wet + Inst dry `dropWavAsClip` calls — so all recordings from a single Record session land with consistent pre-roll alignment.
- **Slip-edit UI affordance (Jeff-locked 2026-05-19 — IN scope for Task 0c):** Builder-grid `ArrangementGrid` (`Source/Standalone/BuilderPage.cpp`) gains slip-edit on EITHER edge of an Audio clip — dragging the left edge decrements `block.contentStartSamples` (reveals pre-roll) and grows `block.lengthBeats` (or increments + shrinks for the trim direction); the clip's right-end and `startBar` stay fixed (FL/Reaper-standard "slip in-point" semantics). Dragging the right edge moves `lengthBeats` only; `contentStartSamples` + `startBeats` stay fixed. **Mode toggle: as-shipped, the Slip/Stretch dropdown in the ArrangementToolbar** (NOT the originally-spec'd `Ctrl+Alt+Home` keybind). During verify-pass-2 a pre-existing `AGTool::SlipEdit` stub was discovered in BuilderPage (toolbar button + `'S'` keybind + tool menu entry, UI fully wired but functionally a no-op since pre-QA-Ea); owner pivoted to retire the parallel `Ctrl+Alt+Home` plumbing in favor of an `EditMode { Slip, Stretch }` dropdown that reuses the existing `'S'` keybind. `mResizeFromLeftEdge` flag deleted; `mEditMode` member + getters/setter/`toggleEditMode()` added; `ArrangementToolbar::mEditModeBtn` wired with `onEditModeRequested` callback + `setEditModeLabel`. `cmdToggleResizeFromLeftEdge` renamed `cmdToggleSlipStretchMode` in KeyBindings (default `'S'`). D-7 (Piano Roll features bundle, `Files For Claude/Keybinds & Feature Plan.txt:96`) will later wire the SAME mode toggle for piano-roll note left-edge resize — Task 0c implements the dropdown + ArrangementGrid surface; D-7 extends it to the Piano-Roll surface.

**Edge cases (Jeff-locked):**
- **User Stops during count-in** (`preRollSamples ≥ totalFileSamples`): `commitRecordingResult::dropWavAsClip` detects `effContentSeconds ≤ 0` and returns early — no visible content to drop; the WAV stays on disk for reference but no clip lands on the grid.
- **No count-in:** `mPreRollSamples` never increments → `preRollSamples = 0` → `contentStartSamples = 0` → existing behavior unchanged (backwards-compatible for every existing project / non-count-in recording).
