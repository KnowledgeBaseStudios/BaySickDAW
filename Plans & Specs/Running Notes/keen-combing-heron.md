...
- `HarmlessEditor.cpp:293` mPartSel comment — **FIXED by badger Task 9** (comment
...
- `Source/Harmless/HarmlessSynth.h` :: HarmlessSynth combined-part 'back-compat' setters (setPrismAmount, setPluckDecay, setBlurSize, setFilterMaskAmount, setPhaserMaskRate, setBrownianAmount, setBlurTime, setBlurHarm, setPrismMode, setPluckBlur)
- `Source/Harmless/HarmlessXYZPad.cpp` :: HarmlessXYZPad::setXY
...
- `Source/Harmless/HarmlessXYZPad.h` :: HarmlessXYZPad::onXYChanged (std::function member)
...
- `Source/Harmless/HarmlessEditor.h` :: mModXDest, mModYDest, mModZDest (+ mModXDestAtt/mModYDestAtt/mModZDestAtt at h:228)
- `Source/Harmless/HarmlessRoutingMatrix.h` :: mToggles[kNumToggles]
...
- **Blocking locks off the audio path** — Harmless mod-registry SpinLock, EffectRack::setHostTransport.
...
- **The vestigial legacy synth.** `mSynth` constructed eight voices at startup and re-prepared them
  ...
  **`SynthSound.h/.cpp` deliberately KEPT** — it sits in the same orphan set and looks like part of
  it, but Harmless genuinely uses it (`AdditiveVoice` dynamic_casts to it, `HarmlessSynth`
  constructs one). Deleting the set wholesale would have taken Harmless down.
...
- Also: the pedalboard save's half-applied fix completed (the guard that made this batch's own
  ...
  symbols removed, with the HarmlessSynth::allNotesOff twin swept per the verifier's "sweep both
...
- **Round 7's honest preset-load message stopped at three of six loaders.** Harmless,
...
- **`Pictures/`** -- 42 PNGs, 7,027,651 bytes, added 2026-08-09, after the documents were written.
  They are named by screen (`Mixer.png`, `Harmless.png`, `BaySickRustyDrums Kick.png`), not by
  `SHOT-` id.
...
1. **The BaySickSynth / BaySickBass filter, LFO and envelopes are pinned to 44100 Hz. CONFIRMED.**
   ...
   holds a `BaySickSynthDSP`, it is the same engine. Harmless and BaySickPlayer are NOT affected;
   ...
   envelopes drift.
2. **Harmless VOL has a discontinuity at the top of its travel. CONFIRMED.** The routing-matrix
   output stage in `HarmlessSynth.cpp` is gated `if (mRmVol != 1.0f || mRmClip > 0.001f)`, and
...
3. **The tremolo / vibrato WAVE icon draws a different wave from the one that plays. CONFIRMED, and
   ...
   `HarmlessWaveformButton` paints positions as 0 sine, 1 saw, 2 square, 3 triangle -- its own
...
- **Harmless's two filter envelopes advanced once per BLOCK**, so wall-clock envelope length was
...
- Harmless filter envelopes advance per sample, with the cutoff applied on a sample-counted
...
12. **The six Task 9 spec calls** (44100-pinned filter/LFO/envelopes on BaySickSynth and BaySickBass,
    the Harmless VOL discontinuity at the top of travel, the tremolo/vibrato WAVE icon order mismatch,
...
what the user chose; delete `hasWarpMap`; smooth the Harmless VOL discontinuity; fix the icon;
...
### Harmless synth engine setter surface
HarmlessSynth is a dual-part additive engine: Part A and Part B are simultaneous layers (never alternate modes), crossfaded by timbre_blend, with per-part spectral modules (prism, pluck decay, blur size/time/harm, filter mask, phaser mask, brownian, prism mode, pluck blur). S3.5 (2026-04-19) split every combined setter into per-part A/B variants; HarmlessProcessor's param plumbing calls ONLY the A/B variants (one caller each). The ten combined setters remain declared "back-compat" but have zero callers. Other live surface confirmed in passing: strum (time/dir/tension applied via applyStrum in the midi path), unison type/alt/phase, part-B shape, phase init, global LFO vol/pitch depth appliers, output phaser + EQ mix, and getAggregatedPartialAmplitudes feeding the central spectrogram at ~30 Hz from the GUI thread (VisualizerScreen::tickFromSynth is called from a header inline - looks dead to naive grep but is live). HarmlessXYZPad: a 3-param pad whose knobs drive APVTS attachments; QA-ModelShell TS3 made attachToApvts stamp-only (applicators registered model-side against the engine APVTS so they survive pad destruction); its programmatic setXY has no callers.
...
### Harmless editor attachment architecture
Two binding families: (1) plain per-param attachments created in the ctor (~60 of them, HarmlessEditor.cpp:376-506); (2) the S3.5 dual-bind family - per-part-able controls (blur_size/blur_time/blur_harm/prism_amt/prism_mode/brownian/pluck_decay/pluck_blur/phaser_mask_rate + partB_* twins) are NOT attached in the ctor; `rebindToPart()` destroys/recreates the live attachment against whichever part is selected, and the header's unique_ptr slots for those params are INTENTIONALLY left null forever (documented at cpp:382-385, 418, 425-426, 430-431, 438). Those null slots look dead to a naive sweep but are documented design. mod_x/y/z attach through HarmlessXYZPad::attachToApvts; mod routing destinations live per-target in HarmlessModRegistry (processor-owned), which the mod editor pulls directly. HarmlessRoutingMatrix: 6 rotary knobs (SUB/PROT/CLIP/FX/VOL/ENV, 16px packed row per Jeff 2026-08-04), attached via attachToApvts with componentID stamped for the Automate menu (applicator registered model-side, outlives the matrix); the design doc's 6 LED toggles were ruled out by Jeff 2026-04-19 and never added.
