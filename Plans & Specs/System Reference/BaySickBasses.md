...
**processor-owned**, not owned by `EngineRig` - it lives in `BaySickDAWProcessor`
...
**Kit loading** (`BaySickDAWProcessor::loadBaySickBassesKit`):
...
3. Create the processor if this is the first load; hand it the transport playhead
   and prepare it.
4. `BaySickBassesProcessor::loadKit` runs the `SafeSfzKit` pre-flight walk over
   the program and everything it pulls in, and parses the SFZ only if that walk
   raises no objection.
...
opcode index above 512, or a NUL byte where text was expected.
...
against the kit's path; the picker's own box (below) names only the file.
