namespace fontaudio
{
  ...
  RenderedIcon IconHelper::getIcon(IconName icon, float size, juce::Colour colour, float scaleFactor)
  {
    ...
    // LOCAL CHANGE (KBS): Font::getStringWidth was deprecated in JUCE 7 and
    // removed in JUCE 8. GlyphArrangement is the supported replacement and
    // measures the same thing. The cast keeps std::max seeing one type.
    scaledSize = std::max ((int) std::ceil (GlyphArrangement::getStringWidth (FontAudio, icon)),
                           scaledSize);
