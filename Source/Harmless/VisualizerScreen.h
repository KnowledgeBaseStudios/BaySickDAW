...
// Real-time 516-partial spectrogram. Polls HarmlessSynth at 30 Hz and renders
// vertical amber bars. Aggregation is done in HarmlessSynth - this component
...
public:
    ...
    void setSynth (const HarmlessSynth* s) noexcept { mSynth = s; }
...
private:
    void timerCallback() override
    {
        ...
        // Aggregator is declared on HarmlessSynth; forward-declared above, so
        // the method call lives in the .cpp? Actually no - this header calls
        // into HarmlessSynth so we need the full type. See VisualizerScreen.cpp
        // for the timer body. (Timer callback is kept out-of-line so we don't
        // pull HarmlessSynth.h into every includer of this header.)
