...
void HarmlessModEditor::makeRotary (juce::Slider& s)
{
...
void HarmlessModEditor::makeBipolar (juce::Slider& s)
{
    makeRotary (s);
    s.setRange (-1.0, 1.0, 0.0);
    s.setDoubleClickReturnValue (true, 0.0);   // center-detent via double-click
    s.getProperties().set (HarmlessLAF::kBipolar, "true");
}

HarmlessModEditor::HarmlessModEditor()
{
    // ── Tab buttons ────────────────────────────────────────────────────────
...
// ── Registry + focus ──────────────────────────────────────────────────────────
void HarmlessModEditor::setRegistry (HarmlessModRegistry* r)
{
    mRegistry = r;
...
void HarmlessModEditor::focusTarget (const juce::String& paramId)
{
    if (! mRegistry) return;
...
void HarmlessModEditor::rebuildTargetDropdown()
{
...
ModTarget* HarmlessModEditor::currentTarget() const noexcept
{
...
ModSourceState* HarmlessModEditor::currentSource() const noexcept
{
...
ModCurveState* HarmlessModEditor::currentCurve() const noexcept
{
    auto* src = currentSource();
...
void HarmlessModEditor::clampActiveTabForCurrentSource()
{
    auto* src = currentSource();
    if (! src) return;
    if (auto* tgt = currentTarget())
...
void HarmlessModEditor::syncControlsFromState()
{
    auto* tgt  = currentTarget();
    auto* src  = currentSource();
...
void HarmlessModEditor::publishEdit() noexcept
{
...
void HarmlessModEditor::sanitizeCurrentCurveTimes() noexcept
{
    auto* curv = currentCurve();
    ...
    std::sort (curv->points.begin(), curv->points.end(),
               [] (const HarmlessCurvePoint& a, const HarmlessCurvePoint& b)
...
void HarmlessModEditor::pushUndo()
{
    auto* curv = currentCurve();
    auto* tgt  = currentTarget();
    auto* src  = currentSource();
...
void HarmlessModEditor::applyFrame (const UndoFrame& f)
{
    if (! mRegistry) return;
...
bool HarmlessModEditor::keyPressed (const juce::KeyPress& k)
{
...
// ── Paint ─────────────────────────────────────────────────────────────────────
void HarmlessModEditor::paint (juce::Graphics& g)
{
    g.setColour (juce::Colour (HarmlessLAF::kChassis));
    g.fillAll();
...
// ── Layout ────────────────────────────────────────────────────────────────────
void HarmlessModEditor::resized()
{
    const int kTopH     = 22;
    const int kTabH     = 22;
    const int kBotH     = 50;
...
// ── Geometry helpers ──────────────────────────────────────────────────────────
juce::Point<float> HarmlessModEditor::pointToPixel (const HarmlessCurvePoint& p) const noexcept
{
    const auto gf = mGraphBounds.toFloat();
...
HarmlessCurvePoint HarmlessModEditor::pixelToPoint (float px, float py) const noexcept
{
    const auto gf = mGraphBounds.toFloat();
    HarmlessCurvePoint pt;
...
int HarmlessModEditor::hitTest (juce::Point<int> pos) const noexcept
{
    auto* curv = currentCurve();
    if (! curv) return -1;
...
// ── Curve render ──────────────────────────────────────────────────────────────
void HarmlessModEditor::drawCurve (juce::Graphics& g) const
{
    auto* curv = currentCurve();
    auto* tgt  = currentTarget();
    auto* src  = currentSource();
    ...
    if (useLfoWave)
    {
        ...
        for (int step = 0; step <= kSteps; ++step)
        {
            ...
            float       tw    = HarmlessModCurve::applySkew (t_raw, skew);
            tw                = HarmlessModCurve::applyPw   (tw, pw);
            const float v = HarmlessModCurve::sampleLfoShape (src->lfoShape, tw);
    ...
    // consecutive pair sampled by the DSP's HarmlessModCurve::sample. This
    ...
    std::vector<HarmlessCurvePoint> sortedPts = curv->points;
    ...
    for (size_t i = 0; i < sortedPts.size(); ++i)
    {
        ...
        for (int s = 1; s < kSegSteps; ++s)
        {
            ...
            const float v = HarmlessModCurve::sample (*curv, t);
...
// ── Scroll bar ────────────────────────────────────────────────────────────────
void HarmlessModEditor::scrollBarMoved (juce::ScrollBar* sb, double newRangeStart)
{
...
void HarmlessModEditor::mouseDown (const juce::MouseEvent& e)
{
...
void HarmlessModEditor::mouseDrag (const juce::MouseEvent& e)
{
    auto* curv = currentCurve();
...
void HarmlessModEditor::mouseUp (const juce::MouseEvent&)
{
    mDragIndex = -1;
...
void HarmlessModEditor::mouseDoubleClick (const juce::MouseEvent& e)
{
    if (mFrozen) return;
    auto* curv = currentCurve();
    if (! curv) return;
