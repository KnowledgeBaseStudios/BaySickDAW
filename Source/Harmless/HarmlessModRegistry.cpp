...
HarmlessModRegistry::HarmlessModRegistry() = default;

const juce::Identifier& HarmlessModRegistry::rootId() noexcept { return kRoot; }

void HarmlessModRegistry::registerTarget (juce::String paramId, juce::String displayName)
{
...
ModTarget* HarmlessModRegistry::findTarget (const juce::String& paramId) noexcept
{
    for (auto& t : mTargets)
...
const ModTarget* HarmlessModRegistry::findTarget (const juce::String& paramId) const noexcept
{
    for (auto& t : mTargets)
        if (t->paramId == paramId) return t.get();
    return nullptr;
...
juce::ValueTree HarmlessModRegistry::toValueTree() const
{
...
void HarmlessModRegistry::fromValueTree (const juce::ValueTree& root)
{
