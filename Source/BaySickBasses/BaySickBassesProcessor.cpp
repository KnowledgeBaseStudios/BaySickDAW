...
void BaySickBassesProcessor::parameterChanged (const juce::String& paramId, float newValue)
{
    ...
    // `bbb_<instIdx>_cc<N>`, so `mCcParamRoot` is the concrete prefix to strip.
...
{
    // L-5 fix (2026-05-05) + Sub-E reversal (2026-05-28 QA-Sfizz): invalid CC
                                            ...
                                            juce::MidiBuffer& midi)
{
    ...
    // 2026-05-06 DSP gate: skip renderBlock when sfizz has no active voices.
    // See BaySickGuitarsProcessor for the reasoning.
