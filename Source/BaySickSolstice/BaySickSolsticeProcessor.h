...
// ── BaySickSolsticeProcessor ─────────────────────────────────────────────────────────
// AudioProcessor wrapper for BaySickSolsticeSynth.
// Owns its own APVTS; param IDs follow the consolidated plan convention:
//   tk_{trackId}_bso_{paramName}
//
// CPU safeguarding:
...
// ─────────────────────────────────────────────────────────────────────────────
class BaySickSolsticeProcessor : public juce::AudioProcessor,
...
public:
    ...
    explicit BaySickSolsticeProcessor (const juce::String& trackId = "lay_0",
    ...
    const juce::String getName()        const override { return "BaySickSolstice"; }
    ...
    // tk_<id>_bso_* edit (full BaySickSolstice internal state - partial values,
    ...
    BaySickSolsticeSynth& getSynth() { return mSynth; }

    // Param prefix used for this instance ("tk_0_bso_", "tk_1_bso_", …).
    ...
    BaySickSolsticeModRegistry& getModRegistry() noexcept { return mModRegistry; }
    const BaySickSolsticeModRegistry& getModRegistry() const noexcept { return mModRegistry; }
...
private:
    ...
    BaySickSolsticeModRegistry mModRegistry;
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (BaySickSolsticeProcessor)
};
