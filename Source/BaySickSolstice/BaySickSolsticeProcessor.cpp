#include "BaySickSolsticeProcessor.h"
#include "SafeXml.h"   // XXE + depth-guarded XML parse (QA-Cleanup)
#include "BaySickSolsticeEditor.h"
#include <cmath>
#include <cstring>

BaySickSolsticeProcessor::BaySickSolsticeProcessor (const juce::String& trackId, juce::UndoManager* undoMgr)
    : juce::AudioProcessor (BusesProperties()
          .withOutput ("Output", juce::AudioChannelSet::stereo(), true)),
      apvts (*this, undoMgr, "BaySickSolsticeState",
             createLayout ("tk_" + trackId + "_bso_")),
      mTrackId (trackId),
      mPrefix  ("tk_" + trackId + "_bso_"),
...
void BaySickSolsticeProcessor::registerModTargets()
{
...
//==============================================================================
bool BaySickSolsticeProcessor::isBusesLayoutSupported (const BusesLayout& layouts) const
{
    return layouts.getMainOutputChannelSet() == juce::AudioChannelSet::stereo()
        || layouts.getMainOutputChannelSet() == juce::AudioChannelSet::mono();
}

void BaySickSolsticeProcessor::prepareToPlay (double sampleRate, int maxBlockSize)
{
    mSynth.prepare (sampleRate, maxBlockSize);
}

juce::AudioProcessorEditor* BaySickSolsticeProcessor::createEditor()
{
    return new BaySickSolsticeEditor (*this);
}

void BaySickSolsticeProcessor::processBlock (juce::AudioBuffer<float>& buffer,
                                       juce::MidiBuffer& midi)
{
    ...
    // -13 LUFS here, so BaySickSolstice comes down 6 dB to sit alongside them.  Applied
...
BaySickSolsticeProcessor::createLayout (const juce::String& p)
{
    ...
    // Was hardcoded to Square in BaySickSolsticeSynth ctor. Now user-selectable via
    ...
    // Cut Self mode: false = Same Pitch, true = Cut All. BaySickSolstice defaults to Same Pitch.
    layout.add (std::make_unique<juce::AudioParameterBool> (
    ...
    // lfo_vel routes outside the registry - see BaySickSolsticeSynth note-on
    ...
    // feed the BaySickSolsticeModRegistry as ModSource::ModX/Y/Z for any target.
    // PRESET-BREAK pre-v1.
...
//==============================================================================
void BaySickSolsticeProcessor::updateFromApvts()
{
        ...
        || lfoTempo != (mCache.lfoTempo != 0))
    {
        ...
        // BaySickSolsticeSynth so its noteOn-velocity-scaling LFO ticks at the
    ...
    // vel→note-on velocity scaling (handled inside BaySickSolsticeSynth).
...
//==============================================================================
void BaySickSolsticeProcessor::getStateInformation (juce::MemoryBlock& dest)
{
    ...
    state.removeChild (state.getChildWithName (BaySickSolsticeModRegistry::rootId()), nullptr);
...
void BaySickSolsticeProcessor::setStateInformation (const void* data, int sz)
{
    if (auto xml = SafeXml::parseBinaryBlob (data, sz))
    {
        if (xml->hasTagName (apvts.state.getType()))
        {
            ...
            auto modTree = tree.getChildWithName (BaySickSolsticeModRegistry::rootId());
            if (modTree.isValid())
