...
public:
    ...
    // ── Engine access ─────────────────────────────────────────────────────────
    // Used by BaySickSolsticeProcessor and (in 3A-Final) BaySickSolsticeEditor to edit
    ...
    // Destination routing moved into BaySickSolsticeModRegistry (per-target mod matrix).
    ...
    // BaySickSolsticeModRegistry that owns the serialized state chunk. All voices
    // cache this pointer and pull snapshots at note-on.
    void setModRegistry (const BaySickSolsticeModRegistry* r) noexcept;
    ...
    // takes effect (previously stored on BaySickSolsticeSynth only with no propagation).
    ...
    //   FX wet / output gain trim / env depth on amp curve. See BaySickSolsticeProcessor
...
private:
    ...
    juce::TimeSliceThread  mBackgroundThread { "BaySickSolsticeRebuild" };
