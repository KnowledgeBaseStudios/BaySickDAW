...
class BaySickSolsticeModRegistry;
struct ModTarget;
struct ModSourceState;
struct ModCurveState;

// ── BaySickSolsticeModEditor (S4 Batch 3 rewrite) ────────────────────────────────────
// Articulator-envelope editor. Operates on a BaySickSolsticeModRegistry:
...
class BaySickSolsticeModEditor : public juce::Component,
                          private juce::ScrollBar::Listener
{
public:
    BaySickSolsticeModEditor();
    ~BaySickSolsticeModEditor() override;

    // Call from BaySickSolsticeEditor ctor. Fills the Articulations dropdown from
    // the registry's registered targets and snaps focus to target 0.
    void setRegistry (BaySickSolsticeModRegistry* r);
...
private:
    ...
    // BaySickSolsticeCurvePoint vs PatternManager::ControlPoint).  What is shared is
    ...
    struct UndoFrame {
        ...
        int          tabIdx    { 0 };
        std::vector<BaySickSolsticeCurvePoint> points;
    };
    ...
    // ── State ─────────────────────────────────────────────────────────────────
    BaySickSolsticeModRegistry* mRegistry       { nullptr };
    int                  mCurrentTargetIdx { -1 };   // index into registry target list
    int                  mDragIndex      { -1 };    // curve point being dragged
    BaySickSolsticeCurvePoint   mDragStartPos   {};        // anchor for shift-axis-snap

    static void makeRotary  (juce::Slider& s);
    static void makeBipolar (juce::Slider& s);
    juce::Point<float>  pointToPixel (const BaySickSolsticeCurvePoint& p) const noexcept;
    BaySickSolsticeCurvePoint  pixelToPoint (float px, float py) const noexcept;
    int                 hitTest       (juce::Point<int> pos) const noexcept;
    void                drawCurve     (juce::Graphics&) const;

    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (BaySickSolsticeModEditor)
};
