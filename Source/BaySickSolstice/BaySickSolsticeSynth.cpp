#include "BaySickSolsticeSynth.h"

BaySickSolsticeSynth::BaySickSolsticeSynth()
{
...
//==============================================================================
void BaySickSolsticeSynth::prepare (double sampleRate, int maxBlockSize)
{
...
void BaySickSolsticeSynth::renderNextBlock (juce::AudioBuffer<float>& buffer,
                                      juce::MidiBuffer& midi)
{
    ...
    // note-off cascade onto the retriggered voice.  BaySickSolstice is always poly.
    {
    ...
    // release = the audible bleed.  BaySickSolstice is always poly.
    if (mCutSelf)
    {
        for (const auto meta : midi)
        {
...
//==============================================================================
void BaySickSolsticeSynth::setUnison (int numVoices, float detuneCents, float spread)
{
    forEachVoice ([&] (AdditiveVoice& v)
        { v.setUnison (numVoices, detuneCents, spread); });
}

void BaySickSolsticeSynth::setPartLevels (float partALevel, float partBLevel)
{
    forEachVoice ([&] (AdditiveVoice& v)
        { v.setPartLevels (partALevel, partBLevel); });
}

void BaySickSolsticeSynth::setAmpEnv (float a, float d, float s, float r)
{
    forEachVoice ([&] (AdditiveVoice& v)
        { v.setAmpEnv (a, d, s, r); });
}

void BaySickSolsticeSynth::setFilterParams (float cutoffHz, float resonance)
{
    forEachVoice ([&] (AdditiveVoice& v)
        { v.setFilterParams (cutoffHz, resonance); });
}

void BaySickSolsticeSynth::setVolume (float vol)
{
    forEachVoice ([&] (AdditiveVoice& v) { v.setVolume (vol); });
}

void BaySickSolsticeSynth::setPan (float pan)
{
    forEachVoice ([&] (AdditiveVoice& v) { v.setPan (pan); });
}

void BaySickSolsticeSynth::setGlide (float glideTimeSec)
{
...
void BaySickSolsticeSynth::setLegato (bool legatoOn)
{
...
void BaySickSolsticeSynth::setPhaseInit (float start, float rand) noexcept
{
    forEachVoice ([&] (AdditiveVoice& v) { v.setPhaseInit (start, rand); });
}

void BaySickSolsticeSynth::setTremoloParams (int shape, float depth, float speed, float gap) noexcept
{
    forEachVoice ([&] (AdditiveVoice& v) { v.setTremoloParams (shape, depth, speed, gap); });
}

void BaySickSolsticeSynth::setVibratoParams (int shape, float depth, float speed, float env) noexcept
{
    forEachVoice ([&] (AdditiveVoice& v) { v.setVibratoParams (shape, depth, speed, env); });
}

void BaySickSolsticeSynth::setFilter2Params (float cutoffHz, float res) noexcept
{
    forEachVoice ([&] (AdditiveVoice& v) { v.setFilter2Params (cutoffHz, res); });
}

void BaySickSolsticeSynth::setPitchOffset (float semitones, float cents) noexcept
{
    forEachVoice ([&] (AdditiveVoice& v) { v.setPitchOffset (semitones, cents); });
}

void BaySickSolsticeSynth::setOutputPhaserParams (float mix, float depth, float rate) noexcept
{
...
void BaySickSolsticeSynth::setOutputPhaserExtras (float width, float ofsHz) noexcept
{
...
void BaySickSolsticeSynth::setPitchFraction (int idx) noexcept
{
...
void BaySickSolsticeSynth::setFilter1Env (float a, float d, float s, float r) noexcept
    { forEachVoice ([&] (AdditiveVoice& v) { v.setFilter1Env (a, d, s, r); }); }
void BaySickSolsticeSynth::setFilter2Env (float a, float d, float s, float r) noexcept
    { forEachVoice ([&] (AdditiveVoice& v) { v.setFilter2Env (a, d, s, r); }); }
void BaySickSolsticeSynth::setFilter1EnvAmt    (float amt)   noexcept { forEachVoice ([amt] (AdditiveVoice& v) { v.setFilter1EnvAmt (amt); }); }
void BaySickSolsticeSynth::setFilter2EnvAmt    (float amt)   noexcept { forEachVoice ([amt] (AdditiveVoice& v) { v.setFilter2EnvAmt (amt); }); }
void BaySickSolsticeSynth::setFilter1CutoffOfs (float semis) noexcept { forEachVoice ([semis] (AdditiveVoice& v) { v.setFilter1CutoffOfs (semis); }); }
void BaySickSolsticeSynth::setFilter2CutoffOfs (float semis) noexcept { forEachVoice ([semis] (AdditiveVoice& v) { v.setFilter2CutoffOfs (semis); }); }
void BaySickSolsticeSynth::setFilter1KbTrack   (float depth) noexcept { forEachVoice ([depth] (AdditiveVoice& v) { v.setFilter1KbTrack (depth); }); }
void BaySickSolsticeSynth::setFilter2KbTrack   (float depth) noexcept { forEachVoice ([depth] (AdditiveVoice& v) { v.setFilter2KbTrack (depth); }); }

// ── 2026-04-19 (S4) Mod XYZ pad input broadcast (dest routing moved to registry)
void BaySickSolsticeSynth::setModXYZ (float x, float y, float z) noexcept
    { forEachVoice ([x, y, z] (AdditiveVoice& v) { v.setModXYZ (x, y, z); }); }

// ── 2026-04-19 (S4 AG-1) Auto-gain mode forwarder ─────────────────────────
void BaySickSolsticeSynth::setAutoGainMode (int mode) noexcept
{
...
void BaySickSolsticeSynth::setModRegistry (const BaySickSolsticeModRegistry* r) noexcept
{
    mModRegistry = r;
    forEachVoice ([r] (AdditiveVoice& v) { v.setModRegistry (r); });
}

void BaySickSolsticeSynth::setBeatsPerSecond (double bps) noexcept
{
...
void BaySickSolsticeSynth::getAggregatedPartialAmplitudes (float* outBuf, int numPartials) const
{
...
void BaySickSolsticeSynth::applyGlobalLfoToAllTargets (int rateIdx, int shape, bool tempoSync)
{
    ...
    const_cast<BaySickSolsticeModRegistry*> (mModRegistry)->publishSnapshot();
}
...
void BaySickSolsticeSynth::applyGlobalLfoVolDepth (float depth) noexcept
{
    // The Volume mod target was registered with paramId pid("volume") in
    // BaySickSolsticeProcessor::registerModTargets - but that paramId already
    // includes the per-track prefix (e.g. "tk_lay_0_bso_volume").  We can't
    ...
    if (mModRegistry == nullptr) return;
    auto* mut = const_cast<BaySickSolsticeModRegistry*> (mModRegistry);
...
void BaySickSolsticeSynth::applyGlobalLfoPitchDepth (float depth) noexcept
{
    if (mModRegistry == nullptr) return;
    auto* mut = const_cast<BaySickSolsticeModRegistry*> (mModRegistry);
    // Thread-safety: audio thread, one leaf-scalar write - no edit lock, for
...
void BaySickSolsticeSynth::setGlobalLfoVelRate (float rateLen, int shape, bool tempoSync) noexcept
{
...
void BaySickSolsticeSynth::tickGlobalLfoVel (int numSamples) noexcept
{
...
void BaySickSolsticeSynth::setBlurTimeA (float t)
{
    mPartA.blur.setTimeScale (t);
    mPartA.requestRebuild();
}
void BaySickSolsticeSynth::setBlurTimeB (float t)
{
    mPartB.blur.setTimeScale (t);
    mPartB.requestRebuild();
}
void BaySickSolsticeSynth::setBlurHarmA (float h)
{
    mPartA.blur.setHarmAxis (h);
    mPartA.requestRebuild();
}
void BaySickSolsticeSynth::setBlurHarmB (float h)
{
...
void BaySickSolsticeSynth::setUnisonType (int type)
{
    forEachVoice ([type] (AdditiveVoice& v) { v.setUnisonType (type); });
}
void BaySickSolsticeSynth::setUnisonAlt (bool on)
{
    forEachVoice ([on] (AdditiveVoice& v) { v.setUnisonAlt (on); });
}
void BaySickSolsticeSynth::setUnisonPhase (float amt)
{
    forEachVoice ([amt] (AdditiveVoice& v) { v.setUnisonPhase (amt); });
}
void BaySickSolsticeSynth::setPartBShape (int shape)
{
...
void BaySickSolsticeSynth::setPrismAmountA      (float a) { mPartA.prism.setAmount (a);     mPartA.requestRebuild(); }
void BaySickSolsticeSynth::setPrismAmountB      (float a) { mPartB.prism.setAmount (a);     mPartB.requestRebuild(); }
void BaySickSolsticeSynth::setPluckDecayA       (float a) { mPartA.pluck.setAmount (a);     mPartA.requestRebuild(); }
void BaySickSolsticeSynth::setPluckDecayB       (float a) { mPartB.pluck.setAmount (a);     mPartB.requestRebuild(); }
void BaySickSolsticeSynth::setBlurSizeA         (float a) { mPartA.blur.setAmount (a);      mPartA.requestRebuild(); }
void BaySickSolsticeSynth::setBlurSizeB         (float a) { mPartB.blur.setAmount (a);      mPartB.requestRebuild(); }
void BaySickSolsticeSynth::setFilterMaskAmountA (float a) { mPartA.filterMask.setAmount(a); mPartA.requestRebuild(); }
void BaySickSolsticeSynth::setFilterMaskAmountB (float a) { mPartB.filterMask.setAmount(a); mPartB.requestRebuild(); }
void BaySickSolsticeSynth::setPhaserMaskRateA   (float r) { mPartA.phaserMask.setRate (r);  mPartA.requestRebuild(); }
void BaySickSolsticeSynth::setPhaserMaskRateB   (float r) { mPartB.phaserMask.setRate (r);  mPartB.requestRebuild(); }
void BaySickSolsticeSynth::setBrownianAmountA   (float a) { mPartA.setBrownianAmount (a);   mPartA.requestRebuild(); }
void BaySickSolsticeSynth::setBrownianAmountB   (float a) { mPartB.setBrownianAmount (a);   mPartB.requestRebuild(); }
...
void BaySickSolsticeSynth::applyStrum (juce::MidiBuffer& midi, int numSamples)
{
...
void BaySickSolsticeSynth::setTimbreBlend (float blend) noexcept
{
    const float b = juce::jlimit (0.f, 1.f, blend);
    forEachVoice ([b] (AdditiveVoice& v) { v.setTimbreBlend (b); });
}

void BaySickSolsticeSynth::setPrismModeA (int mode)
{
    mPartA.prism.setMode (juce::jlimit (0, 2, mode));
    mPartA.requestRebuild();
}
void BaySickSolsticeSynth::setPrismModeB (int mode)
{
    mPartB.prism.setMode (juce::jlimit (0, 2, mode));
    mPartB.requestRebuild();
}

void BaySickSolsticeSynth::setPluckBlurA (bool blurOn)
{
    mPartA.pluck.setBlur (blurOn);
    mPartA.requestRebuild();
}
void BaySickSolsticeSynth::setPluckBlurB (bool blurOn)
{
    mPartB.pluck.setBlur (blurOn);
    mPartB.requestRebuild();
}

void BaySickSolsticeSynth::setOutputEQMix (float mix) noexcept
{
...
void BaySickSolsticeSynth::rebuildTiltEQ()
{
...
void BaySickSolsticeSynth::setFilterType (int t)
{
    const int n = juce::jlimit (0, 3, t);
    forEachVoice ([n] (AdditiveVoice& v) { v.setFilterType (n); });
}

void BaySickSolsticeSynth::setFilter2Type (int t)
{
    const int n = juce::jlimit (0, 3, t);
    forEachVoice ([n] (AdditiveVoice& v) { v.setFilter2Type (n); });
}

void BaySickSolsticeSynth::setRoutingMatrix (float sub, float prot, float clip,
