                                      ...
                                      int startSample, int numSamples)
{
    ...
    // the count made BaySickSolstice get quieter every time a voice was added, while
...
// held here and consumed by the BaySickSolsticeModRegistry per-target mod matrix
// in Batch 2.
    ...
    for (int ti = 0; ti < n; ++ti)
    {
        ...
        // through whatever curve segment wasn't reached yet.
        {
            ...
            if (std::abs (src.depth) > 0.001f)
            {
                ...
                const float raw = BaySickSolsticeModCurve::sample (curve, voiceT.envPhase);
                sum += BaySickSolsticeModCurve::toBipolar (raw, src.depth);
        ...
        // LFO so users learn one concept. src.lfoShape picks the waveform.
        {
            ...
            if (std::abs (src.depth) > 0.001f)
            {
                ...
                if (curve.points.size() > 2)
                    raw = BaySickSolsticeModCurve::sample (curve, voiceT.lfoPhase);
                else
                    raw = BaySickSolsticeModCurve::sampleLfoShape (src.lfoShape, voiceT.lfoPhase);
                sum += BaySickSolsticeModCurve::toBipolar (raw, src.depth);
                blendUni (raw, src.depth);
            }
        }
        ...
        // --- Velocity (static, captured at note-on) ---
        {
            ...
            {
                const auto& curve = src.tabs[(size_t) juce::jlimit (0, (int) src.tabs.size() - 1, src.activeTab)];
                const float raw = BaySickSolsticeModCurve::sample (curve, voiceT.capturedVel);
                sum += BaySickSolsticeModCurve::toBipolar (raw, src.depth);
                blendUni (raw, src.depth);
            }
        }
        // --- Keyboard (static) ---
        {
            ...
            {
                const auto& curve = src.tabs[(size_t) juce::jlimit (0, (int) src.tabs.size() - 1, src.activeTab)];
                const float raw = BaySickSolsticeModCurve::sample (curve, voiceT.capturedKey);
                sum += BaySickSolsticeModCurve::toBipolar (raw, src.depth);
                blendUni (raw, src.depth);
            }
        }
        ...
        for (int si = 0; si < 3; ++si)
        {
            ...
            {
                const auto& curve = src.tabs[(size_t) juce::jlimit (0, (int) src.tabs.size() - 1, src.activeTab)];
                const float raw = BaySickSolsticeModCurve::sample (curve, padVals[si]);
                sum += BaySickSolsticeModCurve::toBipolar (raw, src.depth);
                blendUni (raw, src.depth);
            }
        }
