...
//==============================================================================
bool BaySickBassProcessor::isBusesLayoutSupported (const BusesLayout& layouts) const
{
    return layouts.getMainOutputChannelSet() == juce::AudioChannelSet::stereo()
        || layouts.getMainOutputChannelSet() == juce::AudioChannelSet::mono();
}

void BaySickBassProcessor::prepareToPlay (double sampleRate, int maxBlockSize)
{
    mSynth.prepare (sampleRate, maxBlockSize);
}

juce::AudioProcessorEditor* BaySickBassProcessor::createEditor()
{
    return new BaySickBassEditor (*this);
}

void BaySickBassProcessor::processBlock (juce::AudioBuffer<float>& buffer,
...
//==============================================================================
// Layout identical to BaySickSynthProcessor::createLayout but with bsb_ prefix
// and bass-optimised defaults: voiceMode=Mono, waveform=SAW, glide=0.08
juce::AudioProcessorValueTreeState::ParameterLayout
BaySickBassProcessor::createLayout (const juce::String& p)
{
    juce::AudioProcessorValueTreeState::ParameterLayout layout;
    auto vid = [] (const juce::String& id) { return juce::ParameterID { id, 1 }; };

    // 2026-04-25: master out (parity with BaySickPlayer).
    ...
    // Ring modulation (P3.8)
    layout.add (std::make_unique<juce::AudioParameterBool> (
        vid (p + "ringMod"), "Ring Mod", false));

    // Per-voice analog drift (P3.10)
    layout.add (std::make_unique<juce::AudioParameterFloat> (
    ...
    adsr ("amp_attack",  "Amp Attack",  0.005f);
    adsr ("amp_decay",   "Amp Decay",   0.15f);
    layout.add (std::make_unique<juce::AudioParameterFloat> (
        vid (p + "amp_sustain"), "Amp Sustain",
        juce::NormalisableRange<float> (0.f, 1.f), 0.6f));
    adsr ("amp_release", "Amp Release", 0.20f);
    ...
    layout.add (std::make_unique<juce::AudioParameterFloat> (
        ...
        juce::NormalisableRange<float> (20.f, 20000.f, 0.f, 0.25f), 4000.f)); // bass default: 4k
    ...
    layout.add (std::make_unique<juce::AudioParameterFloat> (
        ...
        juce::NormalisableRange<float> (-1.f, 1.f), 0.3f));
    ...
    // ── Filter ADSR ───────────────────────────────────────────────────────────
    adsr ("flt_attack",  "Filter Attack",  0.005f);
    adsr ("flt_decay",   "Filter Decay",   0.20f);
    layout.add (std::make_unique<juce::AudioParameterFloat> (
        vid (p + "flt_sustain"), "Filter Sustain",
        juce::NormalisableRange<float> (0.f, 1.f), 0.3f));
    adsr ("flt_release", "Filter Release", 0.20f);
...
//==============================================================================
// Identical logic to BaySickSynthProcessor::updateFromApvts
void BaySickBassProcessor::updateFromApvts()
{
    ...
    // 2026-04-25: master out volume (parity with BaySickPlayer).
...
//==============================================================================
void BaySickBassProcessor::getStateInformation (juce::MemoryBlock& dest)
{
