...
void BaySickBassEditor::initKnob (juce::Slider& s, const juce::String& tooltip)
{
...
void BaySickBassEditor::initVSlider (juce::Slider& s, const juce::String& tooltip)
{
...
void BaySickBassEditor::initLabel (juce::Label& l, const juce::String& text)
{
...
void BaySickBassEditor::initGroup (juce::GroupComponent& grp, const juce::String& name)
{
    grp.setText      (name);
...
BaySickBassEditor::BaySickBassEditor (BaySickBassProcessor& p)
    : juce::AudioProcessorEditor (p), mProc (p)
{
    setLookAndFeel (&mBassLAF);
    setSize (480, 440);
    ...
    mDualOscModeCbo.setTooltip ("Dual-osc tuning mode (affects SAW+SAW / SAW+SQUARE / SQUARE+SQUARE only).\n"
                                ...
                                "Absolute Hz: Modifier sets osc2 to a specific Hz.");
    ...
    mOscSyncBtn.setTooltip ("Hard sync: osc2 phase restarts when osc1 completes a cycle.\n"
                            "Classic 80s sync-lead sound. Active only for dual-osc waveforms.\n"
                            "Sweep osc2's pitch (Modifier with Hz modes) for the sync sweep.");
    ...
    mRingModBtn.setTooltip ("Ring modulation: sample = osc1 x osc2.\n"
                            ...
                            "Active only for dual-osc waveforms. Combines with SYNC for extra weirdness.");
    ...
    mLFOShapeLed = std::make_unique<BssLedRadio> (avts, pid ("lfo_shape"),
                       juce::StringArray { "Sine", "Saw", "Square" }, 1, 3, juce::Colour (BaySickBassLAF::kCyan));
    mLFODestLed  = std::make_unique<BssLedRadio> (avts, pid ("lfo_dest"),
                       juce::StringArray { "Filter", "Pitch", "Osc Mod" }, 1, 3, juce::Colour (BaySickBassLAF::kCyan));
    ...
    initKnob  (mDriftKnob, "Analog drift - slow per-voice pitch wander (up to +/-10 cents).\n"
    ...
    //   and Type-in-value menus. Pattern mirrors BaySickPlayer (see BaySickPlayerEditor:203).
...
BaySickBassEditor::~BaySickBassEditor()
{
...
void BaySickBassEditor::valueTreeRedirected (juce::ValueTree& tree)
{
...
void BaySickBassEditor::refreshModifierTooltip()
{
    ...
    switch (mode)
    {
        ...
        default: // Musical
            ...
            mModifierKnob.textFromValueFunction = nullptr;
...
void BaySickBassEditor::refreshLFOSyncEnableState()
{
    bool syncOn = false;
...
//==============================================================================
void BaySickBassEditor::paint (juce::Graphics& g)
{
    g.fillAll (juce::Colour (BaySickBassLAF::kBgMain));
...
//==============================================================================
void BaySickBassEditor::resized()
{
...
static constexpr int kBassLblH  = 14;
static constexpr int kBassPad   = 5;
static constexpr int kBassGrpT  = 16;  // space for GroupComponent title
static constexpr int kBassKnobSz = 80; // target knob size (2026-04-22 layout polish)

void BaySickBassEditor::layoutOscDeck (juce::Rectangle<int> deck)
{
    const int colW = deck.getWidth() / 3;

    // ── Column 1: Waveform group ──────────────────────────────────────────────
    auto col1 = deck.withWidth (colW).reduced (kBassPad);
    mWavGroup.setBounds (col1);

    auto wInner = col1.reduced (4, kBassGrpT);
    const int cboH     = 24;
    ...
    mOscSyncBtn     .setBounds (wInner.getX(),             wInner.getBottom() - syncBtnH, halfW - 2,                syncBtnH);
    mRingModBtn     .setBounds (wInner.getX() + halfW + 2, wInner.getBottom() - syncBtnH, wInner.getWidth() - halfW - 2, syncBtnH);

    const int stackedH = cboH + gap + dualCboH + kBassPad;
    ...
    auto placeKnob = [&] (juce::Slider& s, juce::Label& l, int col)
    {
        ...
        const int kh = juce::jmin (kBassKnobSz, knobArea - kBassLblH - 2);
        const int y  = wInner.getY() + stackedH + ((knobArea - kBassLblH - 2) - kh) / 2;
        s.setBounds (x, y, kW3, kh);
        l.setBounds (x, y + kh, kW3, kBassLblH);
    };
    ...
    auto col2 = deck.withX (deck.getX() + colW).withWidth (colW).reduced (kBassPad);
    mVoiceGroup.setBounds (col2);

    auto vInner = col2.reduced (4, kBassGrpT);
    const int ledH   = 26;
    const int cutH   = 22;
    const int vGap   = 4;
    ...
    const int kSlot    = juce::jmin (kBassKnobSz, juce::jmin (vmHalfW, knobsH - kBassLblH));

    const int slideX  = vInner.getX() + (vmHalfW - kSlot) / 2;
    const int outVolX = vInner.getX() + vmHalfW + (vmHalfW - kSlot) / 2;
    const int knobsY  = knobsTop + ((knobsH - kBassLblH) - kSlot) / 2;

    mGlideKnob .setBounds (slideX,  knobsY, kSlot, kSlot);
    mOutVolKnob.setBounds (outVolX, knobsY, kSlot, kSlot);
    mGlideLbl .setBounds (vInner.getX(),           vInner.getBottom() - kBassLblH, vmHalfW, kBassLblH);
    mOutVolLbl.setBounds (vInner.getX() + vmHalfW, vInner.getBottom() - kBassLblH, vmHalfW, kBassLblH);

    // ── Column 3: Mod Wheel group ─────────────────────────────────────────────
    auto col3 = deck.withX (deck.getX() + colW * 2).withWidth (colW).reduced (kBassPad);
    mModWheelGroup.setBounds (col3);

    auto mwInner = col3.reduced (4, kBassGrpT);
    const int mwLedH = 26;   // horizontal 1×2 grid
    if (mModWheelDestLed) mModWheelDestLed->setBounds (mwInner.getX(), mwInner.getY(), mwInner.getWidth(), mwLedH);

    const int amtH    = mwInner.getHeight() - mwLedH - kBassPad;
    const int amtY    = mwInner.getBottom() - amtH;
    const int aKnobH  = juce::jmin (kBassKnobSz, amtH - kBassLblH);
    const int aKnobX  = mwInner.getX() + (mwInner.getWidth() - aKnobH) / 2;
    const int aKnobY  = amtY + ((amtH - kBassLblH) - aKnobH) / 2;
    mModWheelAmtKnob.setBounds (aKnobX, aKnobY, aKnobH, aKnobH);
    mModWheelAmtLbl .setBounds (mwInner.getX(), amtY + amtH - kBassLblH, mwInner.getWidth(), kBassLblH);
}

void BaySickBassEditor::layoutAmpEnvDeck (juce::Rectangle<int> deck)
{
    // Side-by-side boxes (2026-04-22 layout polish).
    const int halfW = deck.getWidth() / 2;
    const int lH    = kBassLblH + 18;
                          ...
                          juce::Slider& extraKnob, juce::Label& extraLbl)
    {
        ...
        auto inner  = boxArea.reduced (4, kBassGrpT);
        const int knobW  = juce::jmin (kBassKnobSz + 8, inner.getWidth() / 5);
        const int sW     = (inner.getWidth() - knobW) / 4;

        for (int i = 0; i < 4; ++i)
        {
            auto r = inner.withX (inner.getX() + i * sW).withWidth (sW).reduced (kBassPad / 2);
            sliders[i]->setBounds (r.getX(), r.getY(), r.getWidth(), r.getHeight() - lH);
            labels [i]->setBounds (r.getX(), r.getBottom() - kBassLblH, r.getWidth(), kBassLblH);
        }
        auto knobArea = inner.withX (inner.getRight() - knobW).withWidth (knobW).reduced (kBassPad / 2);
        const int kh = juce::jmin (kBassKnobSz, knobArea.getHeight() - kBassLblH - 2);
        const int kY = knobArea.getY() + ((knobArea.getHeight() - kBassLblH - 2) - kh) / 2;
        extraKnob.setBounds (knobArea.getX(), kY, knobArea.getWidth(), kh);
        extraLbl .setBounds (knobArea.getX(), knobArea.getBottom() - kBassLblH, knobArea.getWidth(), kBassLblH);
    };
    ...
    auto ampBox   = deck.withWidth (halfW).reduced (kBassPad);
    auto pitchBox = deck.withTrimmedLeft (halfW).reduced (kBassPad);
...
void BaySickBassEditor::layoutFilterDeck (juce::Rectangle<int> deck)
{
    // XY pad left, right half split vertically: TYPE top / TRACKING below.
    const int xyW    = deck.getWidth() * 50 / 100;
    const int typeH  = deck.getHeight() * 45 / 100;

    auto xyArea    = deck.withWidth (xyW).reduced (kBassPad);
    auto rightCol  = deck.withX (deck.getX() + xyW).withWidth (deck.getWidth() - xyW);
    auto typeArea  = rightCol.withHeight (typeH).reduced (kBassPad);
    auto trackArea = rightCol.withTrimmedTop (typeH).reduced (kBassPad);

    if (mFilterXYPad) mFilterXYPad->setBounds (xyArea);

    mFilterTypeGroup.setBounds (typeArea);
    auto typeInner = typeArea.reduced (4, kBassGrpT);
    ...
    auto tInner = trackArea.reduced (4, kBassGrpT);
    const int kW2 = tInner.getWidth() / 2;
    auto placeKnob = [&] (juce::Slider& s, juce::Label& l, int col)
    {
        auto r = tInner.withX (tInner.getX() + col * kW2).withWidth (kW2);
        const int kh = juce::jmin (kBassKnobSz, r.getHeight() - kBassLblH - 2);
        const int kY = r.getY() + ((r.getHeight() - kBassLblH - 2) - kh) / 2;
        s.setBounds (r.getX(), kY, r.getWidth(), kh);
        l.setBounds (r.getX(), r.getBottom() - kBassLblH, r.getWidth(), kBassLblH);
    };
    placeKnob (mFltKbTrackKnob,  mFltKbTrackLbl,  0);
    placeKnob (mFltVelTrackKnob, mFltVelTrackLbl, 1);
}

void BaySickBassEditor::layoutFltEnvDeck (juce::Rectangle<int> deck)
{
    const int amtW = 80;
    ...
    for (int i = 0; i < 4; ++i)
    {
        auto r = deck.withX (deck.getX() + i * sW).withWidth (sW).reduced (kBassPad / 2);
        sliders[i]->setBounds (r.getX(), r.getY(), r.getWidth(), r.getHeight() - lH);
        labels [i]->setBounds (r.getX(), r.getBottom() - kBassLblH, r.getWidth(), kBassLblH);
    }

    auto amtArea = deck.withX (deck.getRight() - amtW).withWidth (amtW).reduced (kBassPad / 2);
    mFltAmtGroup.setBounds (amtArea);
    auto aInner = amtArea.reduced (4, kBassGrpT);
    const int kh = juce::jmin (kBassKnobSz, aInner.getHeight() - kBassLblH - 2);
    const int kY = aInner.getY() + ((aInner.getHeight() - kBassLblH - 2) - kh) / 2;
    mFltEnvAmtKnob.setBounds (aInner.getX(), kY, aInner.getWidth(), kh);
    mFltEnvAmtLbl .setBounds (aInner.getX(), aInner.getBottom() - kBassLblH, aInner.getWidth(), kBassLblH);
}

void BaySickBassEditor::layoutLFODeck (juce::Rectangle<int> deck)
{
    const int pW = deck.getWidth() / 4;

    // ── Shape ─────────────────────────────────────────────────────────────────
    auto shape = deck.withWidth (pW).reduced (kBassPad);
    mLFOShapeGroup.setBounds (shape);
    if (mLFOShapeLed) mLFOShapeLed->setBounds (shape.reduced (4, kBassGrpT));

    // ── Rate ──────────────────────────────────────────────────────────────────
    auto rate = deck.withX (deck.getX() + pW).withWidth (pW).reduced (kBassPad);
    mLFORateGroup.setBounds (rate);
    auto rInner = rate.reduced (4, kBassGrpT);
    // Stack: Rate knob (top, capped at kBassKnobSz, centered) | RATE | DIV | SYNC
    const int syncH    = 22;
    const int divH     = 20;
    const int gap      = 3;
    const int knobArea = rInner.getHeight() - kBassLblH - divH - syncH - (gap * 2);
    const int knobH    = juce::jmin (kBassKnobSz, knobArea);
    ...
    mLFORateLbl .setBounds (rInner.getX(), y, rInner.getWidth(), kBassLblH); y += kBassLblH + gap;
    ...
    // ── Dest ──────────────────────────────────────────────────────────────────
    auto dest = deck.withX (deck.getX() + pW * 2).withWidth (pW).reduced (kBassPad);
    mLFODestGroup.setBounds (dest);
    if (mLFODestLed) mLFODestLed->setBounds (dest.reduced (4, kBassGrpT));

    // ── Amount ────────────────────────────────────────────────────────────────
    auto amt = deck.withX (deck.getX() + pW * 3).withWidth (pW).reduced (kBassPad);
    mLFOAmtGroup.setBounds (amt);
    auto aInner = amt.reduced (4, kBassGrpT);
    const int kh = juce::jmin (kBassKnobSz, aInner.getHeight() - kBassLblH - 2);
    const int kY = aInner.getY() + ((aInner.getHeight() - kBassLblH - 2) - kh) / 2;
    mLFOAmtKnob.setBounds (aInner.getX(), kY, aInner.getWidth(), kh);
    mLFOAmtLbl .setBounds (aInner.getX(), aInner.getBottom() - kBassLblH, aInner.getWidth(), kBassLblH);
}
...
void BaySickBassEditor::layoutModDeck (juce::Rectangle<int> deck)
{
    ...
    auto noiseArea = deck.withWidth (colW).reduced (kBassPad);
    mNoiseGroup.setBounds (noiseArea);
    auto nInner = noiseArea.reduced (4, kBassGrpT);
    const int btnH = 26;
    const int cboH = 20;
    const int nGap = 6;
    ...
    auto placeKnob = [&] (juce::Slider& s, juce::Label& l, int col, juce::Rectangle<int> area, int slotW)
    {
        ...
        const int kh = juce::jmin (kBassKnobSz, area.getHeight() - kBassLblH - 2);
        const int kY = area.getY() + ((area.getHeight() - kBassLblH - 2) - kh) / 2;
        s.setBounds (x, kY, slotW, kh);
        l.setBounds (x, area.getBottom() - kBassLblH, slotW, kBassLblH);
    };

    auto transArea = deck.withX (deck.getX() + colW).withWidth (colW * 2).reduced (kBassPad);
    mTransientGroup.setBounds (transArea);
    auto tInner = transArea.reduced (4, kBassGrpT);
    ...
    auto burstArea = deck.withX (deck.getX() + colW * 3).withWidth (colW * 2).reduced (kBassPad);
    mBurstGroup.setBounds (burstArea);
    auto bInner = burstArea.reduced (4, kBassGrpT);
    const int burstBtnH = 22;
    mBurstModeBtn.setBounds (bInner.getX(), bInner.getY(), bInner.getWidth(), burstBtnH);
    auto bKnobRow = bInner.withTrimmedTop (burstBtnH + kBassPad);
    ...
    auto driftArea = deck.withX (deck.getX() + colW * 5).withWidth (colW).reduced (kBassPad);
    mDriftGroup.setBounds (driftArea);
    auto dInner = driftArea.reduced (4, kBassGrpT);
    placeKnob (mDriftKnob, mDriftLbl, 0, dInner, dInner.getWidth());

    auto uniArea = deck.withX (deck.getX() + colW * 6).withWidth (colW * 2).reduced (kBassPad);
    mUnisonGroup.setBounds (uniArea);
    auto uInner = uniArea.reduced (4, kBassGrpT);
...
void BaySickBassEditor::showDeck (int deck, bool visible)
{
    switch (deck)
    {
        case 0: // OSC
...
void BaySickBassEditor::setActiveTab (int tab)
{
...
void BaySickBassEditor::updateTabButtons()
{
    for (int i = 0; i < 6; ++i)
    {
                                           ...
                                           std::function<void(const juce::File&)> onSelect)
{
    ...
    {
        juce::PopupMenu sub;
        addBassEditorPresetDirToMenu (sub, child, onSelect);
...
void BaySickBassEditor::showPresetMenu()
{
    juce::PopupMenu menu;
    ...
    if (dir.exists())
        addBassEditorPresetDirToMenu (menu, dir, [this] (const juce::File& f) { loadPreset (f); });
    ...
    menu.addItem ("Save preset...", [this]
    {
        ...
        dlg->addTextEditor ("name", "My Bass Preset");
...
void BaySickBassEditor::savePreset (const juce::String& name)
{
...
void BaySickBassEditor::loadPreset (const juce::File& f)
{
    ...
    for (int i = 0; i < loaded.getNumChildren(); ++i)
    {
        ...
        const int tagIdx = id.indexOf ("_bsb_");
